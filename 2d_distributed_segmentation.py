...
def shrink_labels_2d(plane, threshold):
    """
    Shrink labels in the given 2D plane by `threshold` pixels from
    their boundary. This can help avoid merging labels that only
    barely touch at a thin boundary.
    """
    import numpy as np
...
def block_face_adjacency_graph_2d(faces, nlabels):
    """
    Build a graph of which labels (in face arrays) touch one another
    across block boundaries, for a 2D setting. Then we can merge
    those labels in a single connected‐component analysis.
    """
    import numpy as np
    import scipy.ndimage
    import scipy.sparse
    ...
    # The critical 2D change:
    structure = scipy.ndimage.generate_binary_structure(2, 1)

    all_mappings = []
    for face in faces:
        # Separate the 2-pixel boundary dimension into two 1-pixel slices:
        sl0 = tuple(slice(0, 1) if d == 2 else slice(None) for d in face.shape)
        sl1 = tuple(slice(1, 2) if d == 2 else slice(None) for d in face.shape)

        a = shrink_labels_2d(face[sl0], 1.0)
        b = shrink_labels_2d(face[sl1], 1.0)

        concat_axis = np.argmin(a.shape)
        face_combined = np.concatenate((a, b), axis=concat_axis)

        # `_across_block_label_grouping` will find which labels match across this boundary
