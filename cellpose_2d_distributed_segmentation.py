...
def remove_overlaps(array, crop, overlap, blocksize):
    """Trim overlaps to isolate the central block"""
    ...
    # Debugging
    print(f"After remove_overlaps, array shape: {array.shape}, unique labels: {np.unique(array)}")
...
def block_faces(segmentation):
    ...
    for iii in range(segmentation.ndim):
        ...
        a[iii] = slice(0, 1)
        face_start = segmentation[tuple(a)]
        faces.append(face_start)
        a = [slice(None),] * segmentation.ndim
        a[iii] = slice(-1, None)
        face_end = segmentation[tuple(a)]
        faces.append(face_end)
        # Debugging
        print(f"Axis {iii} start face unique labels: {np.unique(face_start)}")
        print(f"Axis {iii} end face unique labels: {np.unique(face_end)}")
    return faces
...
def adjacent_faces(block_indices, faces):
    ...
    for block_index in block_indices:
        for ax in range(len(block_index)):
            ...
            try:
                a = faces_index_lookup[block_index][2 * ax + 1]  # End face
                b = faces_index_lookup[neighbor_index][2 * ax]    # Start face
                face_pairs.append((a, b))  # Store as tuple, don’t concatenate yet
            except KeyError:
                continue
    return face_pairs
...
def shrink_labels_2d(plane, threshold):
    import numpy as np
    import scipy.ndimage

    plane_squeezed = plane.squeeze()
    if min(plane_squeezed.shape) == 1:
        return plane_squeezed.reshape(plane.shape)

    grad = np.gradient(plane_squeezed)
    gradmag = np.linalg.norm(grad, axis=0)
    shrunk_labels = np.copy(plane_squeezed)
    shrunk_labels[gradmag > 0] = 0
    distances = scipy.ndimage.distance_transform_edt(shrunk_labels)
    shrunk_labels[distances <= threshold] = 0
    return shrunk_labels.reshape(plane.shape)

def block_face_adjacency_graph_2d(faces, nlabels):
    import numpy as np
    import scipy.ndimage
    import scipy.sparse
    import dask_image.ndmeasure
    ...
    all_mappings = []
    for a, b in faces:  # Unpack the face pair

        # Ensure shapes are compatible for concatenation
        a = a.squeeze()
        b = b.squeeze()
        if a.ndim == 1:
            a = a[np.newaxis, :]  # Make 2D: (1, N)
            b = b[np.newaxis, :]  # Make 2D: (1, N)
        elif b.ndim == 1:
            a = a[:, np.newaxis]  # Make 2D: (N, 1)
            b = b[:, np.newaxis]  # Make 2D: (N, 1)

        concat_axis = 0 if a.shape[0] == 1 else 1  # Horizontal or vertical pair
        face_combined = np.concatenate((a, b), axis=concat_axis)

        mapped = dask_image.ndmeasure._utils._label._across_block_label_grouping(
            face_combined, structure
        )

        all_mappings.append(mapped)

    if not all_mappings:
        return scipy.sparse.csr_matrix((nlabels + 1, nlabels + 1), dtype=np.uint8)

    i, j = np.concatenate(all_mappings, axis=1)
    v = np.ones_like(i)
    return scipy.sparse.coo_matrix((v, (i, j)), shape=(nlabels + 1, nlabels + 1)).tocsr()

def determine_merge_relabeling_2d(block_indices, faces, used_labels):
    """Determine boundary segment mergers, remap all label IDs to merge
       and put all label IDs in range [1..N] for N global segments found"""
    import numpy as np
    ...
    label_groups = block_face_adjacency_graph_2d(faces, label_range)
...
def distributed_eval_2d(
    ...
    This function is specialized for *2D data only*. 

    Returns
    -------
    Two values are returned:
    (1) A reference to the zarr array on disk containing the stitched Cellpose
        segments for your entire 2D image.
    (2) Bounding boxes for every segment. This is a list of tuples of 2D slices:
        [
            (slice(y1, y2), slice(x1, x2)), 
            ...
        ]
        The list is sorted according to segment ID: the segment with the 
        smallest ID is the first tuple, and the segment with the largest 
        ID is the last.
    """
    ...
    import numpy as np
    import zarr
    import dask_jobqueue.core

    # These helper functions are assumed to exist in your code base,
    # or to be provided by some other import. They must be 2D-compatible:
    # - get_block_crops(shape, blocksize, overlap, mask)
    # - process_block(...)  [handles reading from input_zarr and writing to output_zarr]
    ...
    # If diameter is not provided, default to 30
    if 'diameter' not in eval_kwargs:
        eval_kwargs['diameter'] = 30

    # Overlap is 2x diameter
    overlap = eval_kwargs['diameter'] * 2

    # Figure out which blocks to process, given the image shape and mask
    block_indices, block_crops = get_block_crops(
        input_zarr.shape, blocksize, overlap, mask
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        temp_zarr_path = os.path.join(temporary_dir, 'segmentation_unstitched.zarr')
        temp_zarr = zarr.open(
            temp_zarr_path, 'w',
            shape=input_zarr.shape,
            chunks=blocksize,
        ...
        # Map each block's processing to the cluster
        futures = cluster.client.map(
            process_block,
            block_indices,
            block_crops,
        ...
        # Optionally scale down the cluster if it's a JobQueueCluster
        if isinstance(cluster, dask_jobqueue.core.JobQueueCluster):
            cluster.scale(0)

        # Unpack results (faces for merging, bounding boxes, and IDs)
        faces, boxes_, box_ids_ = zip(*results)
        boxes = [box for sublist in boxes_ for box in sublist]
        box_ids = np.concatenate(box_ids_).astype(int)

        # Figure out how to merge labels (i.e. handle overlapping regions)
        new_labeling = determine_merge_relabeling_2d(block_indices, faces, box_ids)

        new_labeling_path = os.path.join(temporary_dir, 'new_labeling.npy')
        np.save(new_labeling_path, new_labeling)

        # Optionally change the worker attributes for stitching (if cluster is JobQueue)
        if isinstance(cluster, dask_jobqueue.core.JobQueueCluster):
            cluster.change_worker_attributes(
                min_workers=cluster.locals_store['min_workers'],
                max_workers=cluster.locals_store['max_workers'],
        ...
        # Create a dask array from the temporary unstitched zarr
        segmentation_da = dask.array.from_zarr(temp_zarr)

        # Relabel it using the new labeling
        def relabel_block(block, relabeling_path):
            remap = np.load(relabeling_path)
            return remap[block]

        relabeled = dask.array.map_blocks(
            relabel_block,
            segmentation_da,
            relabeling_path=new_labeling_path,
            dtype=np.uint32,
            chunks=segmentation_da.chunks,
        )

        # Write final stitched segmentation to disk
        dask.array.to_zarr(relabeled, write_path, overwrite=True)

        # Merge bounding boxes according to final label IDs
        merged_boxes = merge_all_boxes(boxes, new_labeling[box_ids])
